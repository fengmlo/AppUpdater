...
import com.chejin360.updatelibrary.R;
...
public class Updater {
    ...
    public static void check(final Context context, final String url, final CheckUpdateRequest request) {
        if (downloadDialog != null && downloadDialog.get() != null) return;
        Net.init(context, url);
...
//                                Toast.makeText(context, "已经是最新版本了", Toast.LENGTH_SHORT).show();
    ...
    public static void check(final Context context, final String url, final CheckUpdateRequest request, final CheckUpdateListener listener) {
